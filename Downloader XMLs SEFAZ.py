...
class App(ctk.CTk):
    ...
    def __init__(self):
        ...
        btn_limpar_links = ctk.CTkButton(frame_final, text="Limpar Baixados", font=("Arial", 14, "bold"), command=self.zerarJson)
        btn_limpar_links.grid(row=0, column=0, padx=5, sticky="we")
        btn_iniciar = ctk.CTkButton(frame_final, text="Iniciar Busca", font=("Arial", 14, "bold"), command=self.iniciar_busca)
    ...
    def cancelarBusca(self):
        self.processo_rodando = False
        self.awaitDownload(mover_arquivos=False)
    ...
    def zerarJson(self, criarArquivo=False):
        if criarArquivo or self.confirmation('Remover Lista de Baixados', 'Essa ação não pode ser desfeita...\n\nDeseja apagar os registros de quais XMLS foram baixados?'):
            jsonInicial = {'baixados': []}
            with open('baixados.json','wt+') as arqivoJson:
                json.dump(jsonInicial, arqivoJson, indent=4)
    ...
    def AdicionarLog(self, log=''):
        with open(f'{os.getcwd()}\log.txt', 'at') as Arquivo:
            Arquivo.write(f'{log}\n')
    def adicionar_baixados(self, id_baixado):
        try:
            with open(f'{os.getcwd()}/baixados.json', 'r', encoding='utf-8') as arquivoJson:
                linksJson = json.load(arquivoJson)
                linksJson['baixados'].append(id_baixado)
                with open('baixados.json','wt+') as links:
                    json.dump(linksJson, links, indent=4)
        except:
            self.zerarJson(criarArquivo=True) # Chama a criação do json, que em tese também o zera
    def get_baixados(self):
        try:
            ...
            self.zerarJson(criarArquivo=True)
